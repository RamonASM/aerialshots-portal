...
DROP POLICY IF EXISTS "Packages viewable by authenticated" ON service_packages;
DROP POLICY IF EXISTS "Packages viewable by authenticated" ON service_packages;
CREATE POLICY "Packages viewable by authenticated" ON service_packages
  FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Packages editable by staff" ON service_packages;
DROP POLICY IF EXISTS "Packages editable by staff" ON service_packages;
...
DROP POLICY IF EXISTS "Package items viewable by authenticated" ON package_items;
DROP POLICY IF EXISTS "Package items viewable by authenticated" ON package_items;
CREATE POLICY "Package items viewable by authenticated" ON package_items
  FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Package items editable by staff" ON package_items;
DROP POLICY IF EXISTS "Package items editable by staff" ON package_items;
...
DROP POLICY IF EXISTS "Package tiers viewable by authenticated" ON package_tiers;
DROP POLICY IF EXISTS "Package tiers viewable by authenticated" ON package_tiers;
CREATE POLICY "Package tiers viewable by authenticated" ON package_tiers
  FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Package tiers editable by staff" ON package_tiers;
DROP POLICY IF EXISTS "Package tiers editable by staff" ON package_tiers;
...
DROP POLICY IF EXISTS "Template conditions manageable by staff" ON template_conditions;
DROP POLICY IF EXISTS "Template conditions manageable by staff" ON template_conditions;
...
DROP POLICY IF EXISTS "Template variables viewable by staff" ON template_variables;
DROP POLICY IF EXISTS "Template variables viewable by staff" ON template_variables;
