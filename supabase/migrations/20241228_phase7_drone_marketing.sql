...
-- ============================================
-- EMAIL TEMPLATES TABLE (dependency for marketing_campaigns)
-- ============================================
...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS airspace_check_id UUID REFERENCES airspace_checks(id);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS drone_approved BOOLEAN DEFAULT NULL;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS drone_notes TEXT;
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Airspace checks are publicly readable" ON airspace_checks;
CREATE POLICY "Airspace checks are publicly readable" ON airspace_checks FOR SELECT
TO public
USING (true);

DROP POLICY IF EXISTS "Authenticated users can create airspace checks" ON airspace_checks;
CREATE POLICY "Authenticated users can create airspace checks" ON airspace_checks FOR INSERT
TO authenticated
WITH CHECK (true);

-- Marketing campaigns - staff only
DROP POLICY IF EXISTS "Staff can manage marketing campaigns" ON marketing_campaigns;
CREATE POLICY "Staff can manage marketing campaigns" ON marketing_campaigns FOR ALL
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Staff can view campaign sends" ON campaign_sends;
CREATE POLICY "Staff can view campaign sends" ON campaign_sends FOR ALL
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Staff can manage marketing lists" ON marketing_lists;
CREATE POLICY "Staff can manage marketing lists" ON marketing_lists FOR ALL
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Staff can manage list members" ON marketing_list_members;
CREATE POLICY "Staff can manage list members" ON marketing_list_members FOR ALL
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
...
CREATE POLICY "Anyone can unsubscribe" ON email_unsubscribes FOR INSERT
TO public
WITH CHECK (true);

DROP POLICY IF EXISTS "Staff can view unsubscribes" ON email_unsubscribes;
CREATE POLICY "Staff can view unsubscribes" ON email_unsubscribes FOR SELECT
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
