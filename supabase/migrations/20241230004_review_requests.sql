...
CREATE INDEX IF NOT EXISTS idx_review_responses_request ON review_responses(review_request_id);
...
DROP POLICY IF EXISTS "Staff can manage review responses" ON review_responses;
CREATE POLICY "Staff can manage review responses" ON review_responses FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');

DROP POLICY IF EXISTS "Staff can manage review templates" ON review_request_templates;
CREATE POLICY "Staff can manage review templates" ON review_request_templates FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');
...
CREATE POLICY "Agents can view their review requests" ON review_requests FOR SELECT
  USING (agent_id = auth.uid());

-- Update timestamp trigger
DROP TRIGGER IF EXISTS update_review_requests_timestamp ON review_requests;
CREATE TRIGGER update_review_requests_timestamp
  BEFORE UPDATE ON review_requests
  FOR EACH ROW
  EXECUTE FUNCTION update_open_house_timestamp();

DROP TRIGGER IF EXISTS update_review_templates_timestamp ON review_request_templates;
CREATE TRIGGER update_review_templates_timestamp
  BEFORE UPDATE ON review_request_templates
  FOR EACH ROW
  EXECUTE FUNCTION update_open_house_timestamp();

DROP TRIGGER IF EXISTS update_review_settings_timestamp ON review_request_settings;
