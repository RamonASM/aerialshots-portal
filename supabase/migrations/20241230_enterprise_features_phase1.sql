...
CREATE TABLE IF NOT EXISTS booking_reference_files (
  ...
  seller_schedule_id UUID, -- FK constraint added later when seller_schedules exists
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'booking_reference_files' AND column_name = 'listing_id') THEN
    ALTER TABLE booking_reference_files ADD COLUMN listing_id UUID REFERENCES listings(id) ON DELETE CASCADE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'booking_reference_files' AND column_name = 'seller_schedule_id') THEN
    -- Add column without FK constraint (seller_schedules may not exist yet)
    ALTER TABLE booking_reference_files ADD COLUMN seller_schedule_id UUID;
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_booking_ref_listing ON booking_reference_files(listing_id);
CREATE INDEX IF NOT EXISTS idx_booking_ref_schedule ON booking_reference_files(seller_schedule_id);
...
-- (Only runs if seller_schedules table exists)
-- =====================================================
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'seller_schedules') THEN
...
CREATE INDEX IF NOT EXISTS idx_waitlist_date ON appointment_waitlist(desired_date);
CREATE INDEX IF NOT EXISTS idx_waitlist_status ON appointment_waitlist(status);
CREATE INDEX IF NOT EXISTS idx_waitlist_territory ON appointment_waitlist(territory_id);
...
CREATE INDEX IF NOT EXISTS idx_travel_fee_order ON travel_fee_calculations(order_id);
CREATE INDEX IF NOT EXISTS idx_travel_fee_listing ON travel_fee_calculations(listing_id);
...
DROP POLICY IF EXISTS "Staff can view business settings" ON business_settings;
CREATE POLICY "Staff can view business settings" ON business_settings FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff
...
CREATE POLICY "Admin can update business settings" ON business_settings FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Staff can view all reference files" ON booking_reference_files;
CREATE POLICY "Staff can view all reference files" ON booking_reference_files FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Agents can view their own reference files" ON booking_reference_files;
CREATE POLICY "Agents can view their own reference files" ON booking_reference_files FOR SELECT
  TO authenticated
  USING (
    listing_id IN (
...
DROP POLICY IF EXISTS "Anyone can insert reference files" ON booking_reference_files;
CREATE POLICY "Anyone can insert reference files" ON booking_reference_files FOR INSERT
  TO authenticated
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "Anyone can read weather forecasts" ON weather_forecasts;
CREATE POLICY "Anyone can read weather forecasts" ON weather_forecasts FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Service can insert weather forecasts" ON weather_forecasts;
CREATE POLICY "Service can insert weather forecasts" ON weather_forecasts FOR INSERT
  TO authenticated
  WITH CHECK (true);
...
DROP POLICY IF EXISTS "Staff can view all waitlist entries" ON appointment_waitlist;
CREATE POLICY "Staff can view all waitlist entries" ON appointment_waitlist FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Agents can view their own waitlist entries" ON appointment_waitlist;
CREATE POLICY "Agents can view their own waitlist entries" ON appointment_waitlist FOR SELECT
  TO authenticated
  USING (
    agent_id IN (
      SELECT id FROM agents WHERE email = auth.jwt() ->> 'email'
...
DROP POLICY IF EXISTS "Anyone can join waitlist" ON appointment_waitlist;
CREATE POLICY "Anyone can join waitlist" ON appointment_waitlist FOR INSERT
  TO authenticated
  WITH CHECK (true);
...
DROP TRIGGER IF EXISTS business_settings_updated ON business_settings;
...
DROP TRIGGER IF EXISTS waitlist_updated ON appointment_waitlist;
