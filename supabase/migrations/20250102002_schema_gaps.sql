...
-- RLS Policies
DROP POLICY IF EXISTS "Agents can view own weekly stats" ON agent_weekly_stats;
CREATE POLICY "Agents can view own weekly stats" ON agent_weekly_stats FOR SELECT
  USING (agent_id IN (
    SELECT id FROM agents WHERE email = auth.jwt() ->> 'email'
  ));

DROP POLICY IF EXISTS "Staff can view all weekly stats" ON agent_weekly_stats;
CREATE POLICY "Staff can view all weekly stats" ON agent_weekly_stats FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Staff can manage weekly stats" ON agent_weekly_stats;
CREATE POLICY "Staff can manage weekly stats" ON agent_weekly_stats FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM staff
      WHERE email = auth.jwt() ->> 'email'
...
-- RLS Policies
DROP POLICY IF EXISTS "Staff can view contact history" ON agent_contact_history;
CREATE POLICY "Staff can view contact history" ON agent_contact_history FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM staff
      WHERE email = auth.jwt() ->> 'email'
...
DROP POLICY IF EXISTS "Staff can manage contact history" ON agent_contact_history;
CREATE POLICY "Staff can manage contact history" ON agent_contact_history FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM staff
      WHERE email = auth.jwt() ->> 'email'
