...
CREATE INDEX IF NOT EXISTS idx_open_house_rsvps_email ON open_house_rsvps(email);
...
CREATE POLICY "Open houses are readable by anyone" ON open_houses FOR SELECT
  USING (true);
...
CREATE POLICY "Anyone can create RSVPs" ON open_house_rsvps FOR INSERT
  WITH CHECK (true);

-- RSVPs are readable by agents who own the open house
DROP POLICY IF EXISTS "Agents can read RSVPs for their open houses" ON open_house_rsvps;
CREATE POLICY "Agents can read RSVPs for their open houses" ON open_house_rsvps FOR SELECT
  USING (
    open_house_id IN (
...
DROP POLICY IF EXISTS "Agents can update RSVPs for their open houses" ON open_house_rsvps;
CREATE POLICY "Agents can update RSVPs for their open houses" ON open_house_rsvps FOR UPDATE
  USING (
    open_house_id IN (
      SELECT id FROM open_houses WHERE agent_id = auth.uid()
    )
...
DROP POLICY IF EXISTS "Staff can update all RSVPs" ON open_house_rsvps;
CREATE POLICY "Staff can update all RSVPs" ON open_house_rsvps FOR UPDATE
  USING (auth.email() LIKE '%@aerialshots.media');
...
DROP TRIGGER IF EXISTS update_open_houses_timestamp ON open_houses;
CREATE TRIGGER update_open_houses_timestamp
  BEFORE UPDATE ON open_houses
  FOR EACH ROW
  EXECUTE FUNCTION update_open_house_timestamp();

DROP TRIGGER IF EXISTS update_open_house_rsvps_timestamp ON open_house_rsvps;
