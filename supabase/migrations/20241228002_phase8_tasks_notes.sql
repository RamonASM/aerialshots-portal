...
DROP POLICY IF EXISTS "Staff can manage job tasks" ON job_tasks;
CREATE POLICY "Staff can manage job tasks" ON job_tasks FOR ALL
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Staff can manage job notes" ON job_notes;
CREATE POLICY "Staff can manage job notes" ON job_notes FOR ALL
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
...
CREATE POLICY "Staff can manage task comments" ON task_comments FOR ALL
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Staff can view task history" ON task_history;
CREATE POLICY "Staff can view task history" ON task_history FOR SELECT
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Staff can manage task templates" ON task_templates;
CREATE POLICY "Staff can manage task templates" ON task_templates FOR ALL
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM staff
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (-- REMOVED: partial index with NOW()) as overdue_tasks,
    CASE
...
BEGIN
  ...
  SELECT
    ...
    COUNT(*) FILTER (-- REMOVED: partial index with NOW()) as overdue
  FROM job_tasks
