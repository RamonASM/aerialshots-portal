...
-- Update trigger
DROP TRIGGER IF EXISTS editor_assignments_updated_at ON editor_assignments;
...
DROP POLICY IF EXISTS "Staff can view all editor assignments" ON editor_assignments;
CREATE POLICY "Staff can view all editor assignments" ON editor_assignments FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));

-- Staff can insert assignments
DROP POLICY IF EXISTS "Staff can create editor assignments" ON editor_assignments;
CREATE POLICY "Staff can create editor assignments" ON editor_assignments FOR INSERT
  WITH CHECK (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));

-- Staff can update assignments
DROP POLICY IF EXISTS "Staff can update editor assignments" ON editor_assignments;
CREATE POLICY "Staff can update editor assignments" ON editor_assignments FOR UPDATE
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));
...
DROP TRIGGER IF EXISTS staff_notification_prefs_updated_at ON staff_notification_preferences;
...
DROP POLICY IF EXISTS "Staff can view activity log" ON ops_activity_log;
CREATE POLICY "Staff can view activity log" ON ops_activity_log FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));

DROP POLICY IF EXISTS "Staff can insert activity log" ON ops_activity_log;
CREATE POLICY "Staff can insert activity log" ON ops_activity_log FOR INSERT
  WITH CHECK (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));
