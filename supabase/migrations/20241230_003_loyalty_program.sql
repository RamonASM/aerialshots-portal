...
-- Indexes
CREATE INDEX idx_loyalty_points_agent ON loyalty_points(agent_id);
CREATE INDEX idx_loyalty_points_type ON loyalty_points(type);
CREATE INDEX idx_loyalty_points_source ON loyalty_points(source, source_id);
CREATE INDEX idx_loyalty_points_expires ON loyalty_points(expires_at) WHERE NOT is_expired;
CREATE INDEX idx_punch_cards_agent ON punch_cards(agent_id);
CREATE INDEX idx_punch_cards_complete ON punch_cards(is_complete) WHERE NOT reward_used;
CREATE INDEX idx_punch_card_punches_card ON punch_card_punches(punch_card_id);
...
CREATE POLICY "Loyalty tiers are public"
  ON loyalty_tiers FOR SELECT
  USING (true);

-- Agents can view their own points
CREATE POLICY "Agents can view their loyalty points"
...
CREATE POLICY "Agents can view their punch cards"
  ON punch_cards FOR SELECT
  USING (agent_id = auth.uid());

-- Staff can manage punch cards
CREATE POLICY "Staff can manage punch cards"
  ON punch_cards FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');

-- Punch card punches follow card access
CREATE POLICY "Punch card punches follow card access"
  ON punch_card_punches FOR SELECT
  USING (
