...
DROP TRIGGER IF EXISTS orders_track_status ON orders;
...
CREATE POLICY "Agents can view own orders" ON orders FOR SELECT
  USING (agent_id IN (
...
CREATE POLICY "Agents can create orders" ON orders FOR INSERT
  WITH CHECK (true);

-- Staff can view all orders
DROP POLICY IF EXISTS "Staff can view all orders" ON orders;
CREATE POLICY "Staff can view all orders" ON orders FOR SELECT
  USING (EXISTS (
...
DROP POLICY IF EXISTS "Users can view order history for their orders" ON order_status_history;
CREATE POLICY "Users can view order history for their orders" ON order_status_history FOR SELECT
  USING (order_id IN (
...
DROP POLICY IF EXISTS "Staff can view all order history" ON order_status_history;
CREATE POLICY "Staff can view all order history" ON order_status_history FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));
