...
DROP POLICY IF EXISTS "Users can view own booking sessions" ON public;
CREATE POLICY "Users can view own booking sessions" ON public.booking_sessions
  FOR SELECT
  USING (
    auth.uid() = user_id
  );
...
DROP POLICY IF EXISTS "Service role has full access to booking sessions" ON public;
CREATE POLICY "Service role has full access to booking sessions" ON public.booking_sessions
  FOR ALL
  USING (auth.jwt()->>'role' = 'service_role');

-- Add updated_at trigger
DROP TRIGGER IF EXISTS set_booking_sessions_updated_at ON public;
