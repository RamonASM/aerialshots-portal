...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_community_amenities_community ON community_amenities(community_id);
CREATE INDEX IF NOT EXISTS idx_community_amenities_category ON community_amenities(category_id);
CREATE INDEX IF NOT EXISTS idx_community_amenities_location ON community_amenities(lat, lng);
CREATE INDEX IF NOT EXISTS idx_community_amenities_tags ON community_amenities USING GIN(tags);
...
DROP POLICY IF EXISTS "Amenity categories are readable by anyone" ON amenity_categories;
CREATE POLICY "Amenity categories are readable by anyone" ON amenity_categories FOR SELECT
  USING (true);

-- Community amenities are readable by anyone
DROP POLICY IF EXISTS "Community amenities are readable by anyone" ON community_amenities;
CREATE POLICY "Community amenities are readable by anyone" ON community_amenities FOR SELECT
  USING (true);

-- Staff can manage amenities
DROP POLICY IF EXISTS "Staff can manage community amenities" ON community_amenities;
CREATE POLICY "Staff can manage community amenities" ON community_amenities FOR ALL
...
DROP POLICY IF EXISTS "Listing amenities are readable by related parties" ON listing_amenities;
CREATE POLICY "Listing amenities are readable by related parties" ON listing_amenities FOR SELECT
  USING (
    listing_id IN (
...
DROP POLICY IF EXISTS "Staff can manage listing amenities" ON listing_amenities;
CREATE POLICY "Staff can manage listing amenities" ON listing_amenities FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');

-- Update timestamp trigger
DROP TRIGGER IF EXISTS update_community_amenities_timestamp ON community_amenities;
