...
-- Indexes
CREATE INDEX idx_drip_campaigns_trigger ON drip_campaigns(trigger_type) WHERE is_active = true;
CREATE INDEX idx_drip_campaign_steps_campaign ON drip_campaign_steps(campaign_id);
...
CREATE INDEX idx_drip_enrollments_next_step ON drip_enrollments(next_step_at) WHERE status = 'active';
CREATE INDEX idx_drip_email_logs_enrollment ON drip_email_logs(enrollment_id);
...
CREATE POLICY "Staff can manage drip campaigns"
  ON drip_campaigns FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');

CREATE POLICY "Staff can manage drip steps"
  ON drip_campaign_steps FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');

CREATE POLICY "Staff can manage drip enrollments"
  ON drip_enrollments FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');

CREATE POLICY "Staff can manage drip logs"
  ON drip_email_logs FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');

-- Contacts can view their own enrollments
CREATE POLICY "Contacts can view their enrollments"
