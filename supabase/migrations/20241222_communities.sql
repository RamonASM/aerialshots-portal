...
CREATE INDEX IF NOT EXISTS idx_communities_location ON communities(lat, lng);
CREATE INDEX IF NOT EXISTS idx_communities_published ON communities(is_published) WHERE is_published = true;
CREATE INDEX IF NOT EXISTS idx_communities_city_state ON communities(city, state);
...
DROP POLICY IF EXISTS "Public can view published communities" ON communities;
CREATE POLICY "Public can view published communities" ON communities FOR SELECT
  USING (is_published = true);

-- Staff can manage communities
DROP POLICY IF EXISTS "Staff can manage communities" ON communities;
CREATE POLICY "Staff can manage communities" ON communities FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM staff
