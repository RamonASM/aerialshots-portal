...
CREATE INDEX IF NOT EXISTS idx_page_views_listing ON page_views(listing_id);
CREATE INDEX IF NOT EXISTS idx_page_views_agent ON page_views(agent_id);
...
CREATE INDEX IF NOT EXISTS idx_media_downloads_listing ON media_downloads(listing_id);
CREATE INDEX IF NOT EXISTS idx_media_downloads_agent ON media_downloads(agent_id);
CREATE INDEX IF NOT EXISTS idx_media_downloads_asset_type ON media_downloads(asset_type);
CREATE INDEX IF NOT EXISTS idx_media_downloads_date ON media_downloads(downloaded_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_lead_conversions_listing ON lead_conversions(listing_id);
CREATE INDEX IF NOT EXISTS idx_lead_conversions_agent ON lead_conversions(agent_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_agent_analytics_summary_id ON agent_analytics_summary(agent_id);
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_listing_analytics_summary_id ON listing_analytics_summary(listing_id);
...
DROP POLICY IF EXISTS "Agents can view own page views" ON page_views;
CREATE POLICY "Agents can view own page views" ON page_views FOR SELECT
  USING (agent_id IN (
    SELECT id FROM agents WHERE email = auth.jwt() ->> 'email'
  ));

-- Staff can view all analytics
DROP POLICY IF EXISTS "Staff can view all page views" ON page_views;
CREATE POLICY "Staff can view all page views" ON page_views FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));

-- Media downloads policies
DROP POLICY IF EXISTS "Anyone can insert downloads" ON media_downloads;
CREATE POLICY "Anyone can insert downloads" ON media_downloads FOR INSERT
  WITH CHECK (true);

DROP POLICY IF EXISTS "Agents can view own downloads" ON media_downloads;
CREATE POLICY "Agents can view own downloads" ON media_downloads FOR SELECT
  USING (agent_id IN (
    SELECT id FROM agents WHERE email = auth.jwt() ->> 'email'
  ));

DROP POLICY IF EXISTS "Staff can view all downloads" ON media_downloads;
CREATE POLICY "Staff can view all downloads" ON media_downloads FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));
...
CREATE POLICY "Anyone can insert lead conversions" ON lead_conversions FOR INSERT
  WITH CHECK (true);

DROP POLICY IF EXISTS "Agents can view own leads" ON lead_conversions;
CREATE POLICY "Agents can view own leads" ON lead_conversions FOR SELECT
  USING (agent_id IN (
    SELECT id FROM agents WHERE email = auth.jwt() ->> 'email'
  ));

DROP POLICY IF EXISTS "Staff can view all leads" ON lead_conversions;
CREATE POLICY "Staff can view all leads" ON lead_conversions FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));
