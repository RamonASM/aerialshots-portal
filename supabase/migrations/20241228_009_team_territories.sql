...
CREATE POLICY "Admin full access to territories"
  ON service_territories FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff
...
CREATE POLICY "Staff can view territories"
  ON service_territories FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff
...
CREATE POLICY "Admin full access to staff_territories"
  ON staff_territories FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff
...
CREATE POLICY "Staff can view staff_territories"
  ON staff_territories FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff
