...
CREATE INDEX idx_render_templates_slug ON render_templates(slug);
CREATE INDEX idx_render_templates_category ON render_templates(category);
CREATE INDEX idx_render_templates_status ON render_templates(status);
CREATE INDEX idx_render_templates_extends ON render_templates(extends_slug);
CREATE INDEX idx_render_templates_agent ON render_templates(agent_id);
CREATE INDEX idx_render_templates_public ON render_templates(is_public) WHERE is_public = TRUE;
...
CREATE INDEX idx_render_template_versions_template ON render_template_versions(template_id);

-- =====================
-- 3. TEMPLATE SETS
-- Groups of templates for carousels/campaigns
-- =====================
CREATE TABLE render_template_sets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  slug TEXT UNIQUE NOT NULL,
  name TEXT NOT NULL,
  description TEXT,
...
CREATE INDEX idx_render_template_sets_slug ON render_template_sets(slug);
CREATE INDEX idx_render_template_sets_type ON render_template_sets(set_type);
CREATE INDEX idx_render_template_sets_archetype ON render_template_sets(story_archetype);

-- =====================
-- 4. TEMPLATE SET ITEMS
-- Junction table for set -> template relationship
-- =====================
CREATE TABLE render_template_set_items (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_render_template_set_items_set ON render_template_set_items(set_id);
CREATE INDEX idx_render_template_set_items_template ON render_template_set_items(template_id);
...
CREATE INDEX idx_render_jobs_agent ON render_jobs(agent_id);
CREATE INDEX idx_render_jobs_listing ON render_jobs(listing_id);
CREATE INDEX idx_render_jobs_campaign ON render_jobs(campaign_id);
CREATE INDEX idx_render_jobs_carousel ON render_jobs(carousel_id);
CREATE INDEX idx_render_jobs_template ON render_jobs(template_id);
CREATE INDEX idx_render_jobs_created ON render_jobs(created_at DESC);
CREATE INDEX idx_render_jobs_queued ON render_jobs(queued_at) WHERE status = 'pending';

-- =====================
-- 6. RENDER JOB SLIDES
-- Individual slide tracking for carousel jobs
-- =====================
CREATE TABLE render_job_slides (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_render_job_slides_job ON render_job_slides(job_id);
CREATE INDEX idx_render_job_slides_status ON render_job_slides(status);
CREATE INDEX idx_render_job_slides_position ON render_job_slides(job_id, position);

-- =====================
-- 7. RENDER FONTS
-- Custom fonts per agent
-- =====================
CREATE TABLE render_fonts (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  agent_id UUID REFERENCES agents(id) ON DELETE CASCADE,
...
CREATE UNIQUE INDEX idx_render_fonts_unique ON render_fonts(agent_id, family, weight, style);
CREATE INDEX idx_render_fonts_agent ON render_fonts(agent_id);
CREATE INDEX idx_render_fonts_family ON render_fonts(family);
...
CREATE INDEX idx_render_cache_template ON render_cache(template_id);
CREATE INDEX idx_render_cache_key ON render_cache(cache_key);
CREATE INDEX idx_render_cache_expires ON render_cache(expires_at);
CREATE INDEX idx_render_cache_status ON render_cache(render_status);

-- =====================
-- 9. TEMPLATE ANALYTICS
-- Track template usage and performance
-- =====================
CREATE TABLE render_template_analytics (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  template_id UUID NOT NULL REFERENCES render_templates(id) ON DELETE CASCADE,
...
CREATE INDEX idx_render_template_analytics_template ON render_template_analytics(template_id);
CREATE INDEX idx_render_template_analytics_event ON render_template_analytics(event_type);
CREATE INDEX idx_render_template_analytics_created ON render_template_analytics(created_at);
...
CREATE POLICY "Published templates viewable by authenticated"
  ON render_templates FOR SELECT
  TO authenticated
  USING (
    status = 'published'
...
CREATE POLICY "Published template sets viewable"
  ON render_template_sets FOR SELECT
  TO authenticated
  USING (status = 'published' OR is_system = TRUE);

CREATE POLICY "Staff can manage template sets"
  ON render_template_sets FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff WHERE auth_user_id = auth.uid()
...
CREATE POLICY "Agents can view own render jobs"
...
CREATE POLICY "Staff can manage render jobs"
...
CREATE POLICY "View render job slides"
  ON render_job_slides FOR SELECT
  TO authenticated
  USING (
    EXISTS (
...
CREATE POLICY "Staff can manage render job slides"
  ON render_job_slides FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff WHERE auth_user_id = auth.uid()
...
CREATE POLICY "Agents can manage own fonts"
  ON render_fonts FOR ALL
  TO authenticated
  USING (
    agent_id IN (
      SELECT id FROM agents WHERE auth_user_id = auth.uid()
...
CREATE POLICY "Cache accessible to authenticated"
  ON render_cache FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Staff can manage cache"
  ON render_cache FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff WHERE auth_user_id = auth.uid()
...
CREATE POLICY "Staff can view analytics"
  ON render_template_analytics FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff WHERE auth_user_id = auth.uid()
...
CREATE POLICY "Staff can insert analytics"
  ON render_template_analytics FOR INSERT
  TO authenticated
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM staff WHERE auth_user_id = auth.uid()
...
CREATE POLICY "View template versions"
  ON render_template_versions FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Staff can manage template versions"
  ON render_template_versions FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff WHERE auth_user_id = auth.uid()
...
CREATE POLICY "View template set items"
  ON render_template_set_items FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Staff can manage template set items"
  ON render_template_set_items FOR ALL
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff WHERE auth_user_id = auth.uid()
...
CREATE POLICY "Authenticated users can read render assets"
  ON storage.objects FOR SELECT
  TO authenticated
  USING (bucket_id = 'render-assets');

CREATE POLICY "Staff can upload render assets"
...
CREATE POLICY "Anyone can read render output"
  ON storage.objects FOR SELECT
  TO public
  USING (bucket_id = 'render-output');

CREATE POLICY "Service role can upload render output"
  ON storage.objects FOR INSERT
  TO authenticated
