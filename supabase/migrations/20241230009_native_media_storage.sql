...
DROP POLICY IF EXISTS "Staff can manage all uploads" ON media_uploads;
CREATE POLICY "Staff can manage all uploads" ON media_uploads
FOR ALL
USING (
...
DROP POLICY IF EXISTS "Agents can view own listing uploads" ON media_uploads;
CREATE POLICY "Agents can view own listing uploads" ON media_uploads
FOR SELECT
USING (
  EXISTS (
...
DROP POLICY IF EXISTS "Staff can manage migration jobs" ON media_migration_jobs;
CREATE POLICY "Staff can manage migration jobs" ON media_migration_jobs
FOR ALL
USING (
  auth.jwt() ->> 'email' LIKE '%@aerialshots.media'
);
