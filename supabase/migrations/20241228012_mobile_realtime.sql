...
DROP POLICY IF EXISTS "Users can manage their own offline actions" ON offline_actions;
CREATE POLICY "Users can manage their own offline actions" ON offline_actions
  FOR ALL
  USING (user_id = auth.uid());

-- Users can view their own notification history
DROP POLICY IF EXISTS "Users can view their own notification history" ON push_notification_history;
CREATE POLICY "Users can view their own notification history" ON push_notification_history
  FOR SELECT
  USING (user_id = auth.uid());
