...
DROP POLICY IF EXISTS "Staff can upload staged photos" ON storage;
CREATE POLICY "Staff can upload staged photos" ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (
...
DROP POLICY IF EXISTS "Staff can read staged photos" ON storage;
CREATE POLICY "Staff can read staged photos" ON storage.objects FOR SELECT
TO authenticated
USING (
  bucket_id = 'staged-photos' AND
  EXISTS (SELECT 1 FROM public.staff WHERE id::text = auth.uid()::text)
...
DROP POLICY IF EXISTS "Staff can delete staged photos" ON storage;
CREATE POLICY "Staff can delete staged photos" ON storage.objects FOR DELETE
TO authenticated
USING (
  bucket_id = 'staged-photos' AND
  EXISTS (SELECT 1 FROM public.staff WHERE id::text = auth.uid()::text)
...
DROP POLICY IF EXISTS "Staff can read processed photos" ON storage;
CREATE POLICY "Staff can read processed photos" ON storage.objects FOR SELECT
TO authenticated
USING (
...
DROP POLICY IF EXISTS "Staff can manage edited photos" ON storage;
CREATE POLICY "Staff can manage edited photos" ON storage.objects FOR ALL
TO authenticated
USING (
...
DROP POLICY IF EXISTS "Anyone can read approved photos" ON storage;
CREATE POLICY "Anyone can read approved photos" ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'approved-photos');

DROP POLICY IF EXISTS "Staff can upload approved photos" ON storage;
CREATE POLICY "Staff can upload approved photos" ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (
...
DROP POLICY IF EXISTS "Staff can delete approved photos" ON storage;
CREATE POLICY "Staff can delete approved photos" ON storage.objects FOR DELETE
TO authenticated
USING (
  bucket_id = 'approved-photos' AND
  EXISTS (SELECT 1 FROM public.staff WHERE id::text = auth.uid()::text)
...
DROP POLICY IF EXISTS "Staff can manage lightroom exports" ON storage;
CREATE POLICY "Staff can manage lightroom exports" ON storage.objects FOR ALL
TO authenticated
USING (
...
DROP POLICY IF EXISTS "Staff can manage inpainting masks" ON storage;
CREATE POLICY "Staff can manage inpainting masks" ON storage.objects FOR ALL
TO authenticated
USING (
