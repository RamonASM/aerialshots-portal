...
DROP POLICY IF EXISTS "Staff can update own location" ON photographer_locations;
CREATE POLICY "Staff can update own location" ON photographer_locations
  FOR ALL
  USING (
    EXISTS (
...
DROP POLICY IF EXISTS "Agents can view photographer location for their listings" ON photographer_locations;
CREATE POLICY "Agents can view photographer location for their listings" ON photographer_locations
  FOR SELECT
  USING (
    listing_id IN (
...
DROP POLICY IF EXISTS "Staff can view all locations" ON photographer_locations;
CREATE POLICY "Staff can view all locations" ON photographer_locations
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM staff s
...
DROP POLICY IF EXISTS "Service role can manage all locations" ON photographer_locations;
CREATE POLICY "Service role can manage all locations" ON photographer_locations
  FOR ALL
