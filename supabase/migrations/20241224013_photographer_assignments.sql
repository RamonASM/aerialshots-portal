...
-- Update trigger
DROP TRIGGER IF EXISTS photographer_assignments_updated_at ON photographer_assignments;
...
DROP POLICY IF EXISTS "Staff can view all photographer assignments" ON photographer_assignments;
CREATE POLICY "Staff can view all photographer assignments" ON photographer_assignments FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));

-- Staff can insert assignments
DROP POLICY IF EXISTS "Staff can create photographer assignments" ON photographer_assignments;
CREATE POLICY "Staff can create photographer assignments" ON photographer_assignments FOR INSERT
  WITH CHECK (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));

-- Staff can update assignments
DROP POLICY IF EXISTS "Staff can update photographer assignments" ON photographer_assignments;
CREATE POLICY "Staff can update photographer assignments" ON photographer_assignments FOR UPDATE
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));
