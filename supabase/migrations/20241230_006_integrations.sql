...
CREATE INDEX idx_agent_mls_credentials_agent ON agent_mls_credentials(agent_id);
CREATE INDEX idx_dropbox_monitors_agent ON dropbox_folder_monitors(agent_id);
CREATE INDEX idx_slack_preferences_agent ON agent_slack_preferences(agent_id);
...
CREATE POLICY "Agents can manage their integrations"
...
CREATE POLICY "Agents can access their OAuth states"
  ON canva_oauth_states FOR ALL
  USING (agent_id = auth.uid())
  WITH CHECK (agent_id = auth.uid());

-- MLS providers are public
CREATE POLICY "MLS providers are public"
  ON mls_providers FOR SELECT
  USING (true);

-- Staff can manage MLS providers
CREATE POLICY "Staff can manage MLS providers"
  ON mls_providers FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');

-- Agents can manage their MLS credentials
CREATE POLICY "Agents can manage their MLS credentials"
  ON agent_mls_credentials FOR ALL
  USING (agent_id = auth.uid())
  WITH CHECK (agent_id = auth.uid());

-- Agents can manage their Dropbox monitors
CREATE POLICY "Agents can manage their Dropbox monitors"
  ON dropbox_folder_monitors FOR ALL
  USING (agent_id = auth.uid())
  WITH CHECK (agent_id = auth.uid());

-- Staff can manage Slack workspaces
CREATE POLICY "Staff can manage Slack workspaces"
  ON slack_workspaces FOR ALL
  USING (auth.email() LIKE '%@aerialshots.media')
  WITH CHECK (auth.email() LIKE '%@aerialshots.media');

-- Agents can manage their Slack preferences
CREATE POLICY "Agents can manage their Slack preferences"
  ON agent_slack_preferences FOR ALL
  USING (agent_id = auth.uid())
  WITH CHECK (agent_id = auth.uid());
