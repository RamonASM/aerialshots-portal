...
DROP POLICY IF EXISTS "Staff can manage seller access controls" ON seller_access_controls;
CREATE POLICY "Staff can manage seller access controls" ON seller_access_controls
  FOR ALL
  USING (
    auth.uid() IN (
...
DROP POLICY IF EXISTS "Agents can manage access for their listings" ON seller_access_controls;
CREATE POLICY "Agents can manage access for their listings" ON seller_access_controls
  FOR ALL
  USING (
    listing_id IN (
...
DROP POLICY IF EXISTS "Staff can manage reschedule requests" ON reschedule_requests;
CREATE POLICY "Staff can manage reschedule requests" ON reschedule_requests
  FOR ALL
  USING (
    auth.uid() IN (
      SELECT auth_user_id FROM staff WHERE is_active = true
...
DROP POLICY IF EXISTS "Agents can view reschedule requests for their listings" ON reschedule_requests;
CREATE POLICY "Agents can view reschedule requests for their listings" ON reschedule_requests
  FOR SELECT
  USING (
    listing_id IN (
      SELECT l.id FROM listings l
...
DROP TRIGGER IF EXISTS seller_access_controls_updated_at ON seller_access_controls;
...
DROP TRIGGER IF EXISTS reschedule_requests_updated_at ON reschedule_requests;
