...
CREATE INDEX IF NOT EXISTS idx_marketing_assets_bannerbear ON marketing_assets(bannerbear_uid);
...
DROP POLICY IF EXISTS "Agents can view own marketing assets" ON marketing_assets;
CREATE POLICY "Agents can view own marketing assets" ON marketing_assets FOR SELECT
  USING (agent_id IN (
    SELECT id FROM agents WHERE email = auth.jwt() ->> 'email'
  ));

-- Staff can view all marketing assets
DROP POLICY IF EXISTS "Staff can view all marketing assets" ON marketing_assets;
CREATE POLICY "Staff can view all marketing assets" ON marketing_assets FOR SELECT
  USING (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));

-- Staff can insert marketing assets
DROP POLICY IF EXISTS "Staff can insert marketing assets" ON marketing_assets;
CREATE POLICY "Staff can insert marketing assets" ON marketing_assets FOR INSERT
  WITH CHECK (EXISTS (
    SELECT 1 FROM staff WHERE email = auth.jwt() ->> 'email'
  ));

-- Allow service role (webhooks) to update assets
DROP POLICY IF EXISTS "Service role can update marketing assets" ON marketing_assets;
CREATE POLICY "Service role can update marketing assets" ON marketing_assets FOR UPDATE
  USING (true);
