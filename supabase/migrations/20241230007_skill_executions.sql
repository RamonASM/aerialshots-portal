...
CREATE INDEX IF NOT EXISTS idx_skill_usage_period ON skill_usage(period_start, period_end);
...
DROP POLICY IF EXISTS "Staff can view all skill executions" ON skill_executions;
CREATE POLICY "Staff can view all skill executions" ON skill_executions FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM staff
...
DROP POLICY IF EXISTS "Agents can view their listing skill outputs" ON listing_skill_outputs;
CREATE POLICY "Agents can view their listing skill outputs" ON listing_skill_outputs FOR SELECT
  TO authenticated
  USING (
    EXISTS (
...
DROP POLICY IF EXISTS "Agents can manage their skill configs" ON agent_skill_configs;
CREATE POLICY "Agents can manage their skill configs" ON agent_skill_configs FOR ALL
  TO authenticated
  USING (
    EXISTS (
...
DROP POLICY IF EXISTS "Agents can view their skill usage" ON skill_usage;
CREATE POLICY "Agents can view their skill usage" ON skill_usage FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM agents
...
DROP TRIGGER IF EXISTS listing_skill_outputs_updated ON listing_skill_outputs;
...
DROP TRIGGER IF EXISTS track_skill_usage_trigger ON skill_executions;
