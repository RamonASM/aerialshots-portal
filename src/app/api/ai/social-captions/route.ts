...
export async function POST(request: NextRequest) {
  try {
    ...
    const { listing_id, address, city, beds, baths, sqft, price, features } = body
    ...
    const prompt = aiPrompts.socialCaptions({
      address,
    ...
    const result = await generateWithAI({ prompt, maxTokens: 1500 })
    ...
    try {
      ...
      captions = match ? JSON.parse(match[0]) : { error: result.content }
    }
    ...
    await supabase.from('credit_transactions').insert({
      ...
      amount: -CREDIT_COST,
      type: 'redemption',
      description: 'Social Media Caption Pack',
    })
    ...
    await supabase.from('ai_tool_usage').insert({
      ...
      output: JSON.parse(JSON.stringify({ captions })),
    ...
    console.error('AI social captions error:', error)
